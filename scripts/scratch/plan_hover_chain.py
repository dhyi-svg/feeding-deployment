...
HANDLE_POS = np.array([0.63249404, -0.17586458, 0.46044464])
HANDLE_QUAT = np.array([0.5, -0.5, -0.5, 0.5])
BACKOFF = float(sys.argv[1]) if len(sys.argv) > 1 else 0.30
N_WAY = int(sys.argv[2]) if len(sys.argv) > 2 else 12
OUT = sys.argv[3] if len(sys.argv) > 3 else "/tmp/hover_chain_plan.json"


def geometry_corrected_quat(base_quat, old_axis, approach_vec):
...
final_quat = geometry_corrected_quat(HANDLE_QUAT, [0, 0, 1], approach_vec).as_quat()
...
for step in range(1, N_WAY + 1):
    ...
    ok = err < 0.02 and delta < 25.0
    all_ok = all_ok and ok
    status = "OK" if ok else "BAD"
    print(f"  step {step}/{N_WAY}: pos={np.round(way_pos, 4)} ik_err={err*100:.2f}cm joint_delta={delta:.1f}deg {status}")
    waypoints.append({"pos": way_pos.tolist(), "quat": joints.tolist(), "joints": joints.tolist(), "err": err, "delta": delta})
    seed = joints
...
with open(OUT, "w") as f:
    json.dump({"waypoints": [w["joints"] for w in waypoints], "positions": [w["pos"] for w in waypoints], "all_ok": bool(all_ok)}, f)
