...
export default {
  ...
  methods: {
    ...
    startCountdown() {
      this.countdownInterval = setInterval(() => {
        if (this.countdown > 0) {
          ...
          this.handleButtonClick(); // 倒计时结束后执行跳转逻辑
        }
      }, 1000);
    },
    updateCountdownText() {
    ...
    //     if (this.countdown === 0) {
    //       clearInterval(this.countdownInterval);
    //       this.autoClickBiteButton();
    //     }
    //   }, 1000);
    // },
    autoClickBiteButton() {
      this.handleButtonClick(); // Programmatically trigger the click event
    },
    handleButtonClickR() {
...
.button3.active {
  border: 3px solid black !important; /* 添加黑色边框 */
}
.button2 {
  background-color: #FFE699;
  border-radius: 20px;
