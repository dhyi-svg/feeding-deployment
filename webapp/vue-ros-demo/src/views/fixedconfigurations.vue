<template>
  ...
  <div class="content">
    <div class="content-body">
      <!-- 颠倒 -->
      <div class="right">
        <!--        <span class="right-first-title2">What is your preferred bite ordering for this plate?</span>-->
        ...
        <div class="right-other-option-text">
          <span class="right-little-text">What would you like to edit?</span>
          <!--          <span class="right-other-option-little-text">Type/Speak your custom commands</span>-->
        </div>
...
<!--              <button @click="startSpeechRecognition" class="voice-start-button" :disabled="isRecognizing">-->
        ...
        <div class="option-container">
            <textarea
              ...
              readonly
            ></textarea>
<!--          <button @click="confirmUpdate" class="confirm-update-button">Confirm Update</button>-->
        </div>
      </div>
      <div class="left">
...
export default {
  data () {
    return {
      ...
      isRecognizing: false,
      customOrder: '',
      videoFrame: null, // 存储图片或视频帧数据
      isVideo: false, // 标识当前是图片还是视频
    }
  },
  watch: {
    transcript(newValue) {
  ...
  methods: {
    sendToRosFromTextBox() {
      if (this.publisher && this.transcript.trim() !== '') {
        ...
        console.log('Message sent to ROS:', this.transcript);
        this.transcript = ''; // 清空输入框
      } else {
      }
    },
    initRosConnection() {
    ...
    confirmSelection() {
      ...
      // } else {
      //   alert('Please select or enter your preferences');
      // }
      this.transcript = '';
      this.$router.push('/ada');
    },
    confirmUpdate(){
      const message = new ROSLIB.Message({
        data: JSON.stringify({ // 将消息内容转换为JSON字符串
          state: 'adap',
          status: 'save' // 使用输入框的内容作为status字段的值
        })
      })
      this.publisher.publish(message)
    },
