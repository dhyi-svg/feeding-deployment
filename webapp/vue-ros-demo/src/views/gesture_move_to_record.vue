  ...
  methods: {
    handleRosMessage(message) {
      ...
      try {
        ...
        if (parsedMessage.state === 'gesture_record' && parsedMessage.status === 'completed') {
          this.$router.push('/gesture_record_positive');
        } else if (parsedMessage.state === 'gesture_test' && parsedMessage.status === 'completed') {
          this.$router.push('/gesture_test');}
      } catch (error) {
      }
    },
    toggleSettings() {
    ...
    initSubscriber() {
      ...
      this.listener.subscribe((msg) => {
        ...
        try {
          ...
          if (parsedMessage.state === 'gesture_record_positive' && parsedMessage.status === 'moved') {
            this.$router.push('/gesture_record_positive'); 
