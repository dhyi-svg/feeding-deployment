...
export default {
  ...
  //   if (this.listener && this.listener.ros) {
  //     setTimeout(() => {
  //       this.listener.ros.close();
  //     }, 500); // 延迟500ms关闭连接
  //   }
  //   next();
  // },
        ...
        try {
          ...
          if (route) {
            if (typeof route === 'string') {
              this.$router.push(route); // string：page name
            } else if (typeof route === 'object') {
              this.$router.push(route); // object：command status
            }
          }
          if (parsedMessage.state === 'bite_pickup' && parsedMessage.status === 'completed') {
            this.$router.push('/transfermeal'); // jump
          }
        } catch (error) {
